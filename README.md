# README
> [!note]
> 虽然仓库是 luigits.github.io，但实际使用的是 Note Library 的文件。
> 也没什么高明的用法，只是给 Note Library 增加了一个远程链接而已。
...
使用 Writerside 插件进行群组编译，可以浏览[build-groups](Writerside/topics/lean-writerside/配置-build-groups.md)

Writerside 支持使用 build groups 来编译多个实例到一个输出目录。

Writerside 可以搭配 Github Actions 来自动编译文档。可以浏览[Github-Actions](Writerside/topics/lean-writerside/Github-Actions.md)~~但我还不知道如何做，先手动编译提交到 Github Pages 的仓库吧。~~

~~运气不错，在复制的仓库中实现了。倘若将其在主仓库也实现，我将会修改 README~~

目前已经实现将仓库 push 到 GitHub 时自动触发 Action 进行部署。
